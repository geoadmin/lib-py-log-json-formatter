...
class FlattenDjangoRequest(logging.Filter):

    def filter(self, record):
        if 'request' not in record.__dict__:
            return True
        if not isinstance(record.request, HttpRequest):
            return True

        self._flatten_request(record, record.request)

        return True

    def _flatten_request(self, record, request):
        self._flatten_dict(record, 'request', request.__dict__)

    def _flatten_dict(self, record, prefix, dct):
        for key, value in dct.items():
            if key.startswith('_'):
                # ignore private attribute
                continue
            if hasattr(value, '__dict__'):
                self._flatten_dict(record, '{}.{}'.format(prefix, key), value.__dict__)
            elif isinstance(value, (dict)):
                self._flatten_dict(record, '{}.{}'.format(prefix, key), value)
            else:
                setattr(record, '{}.{}'.format(prefix, key), value)
...
class JsonDjangoRequest(logging.Filter):
    ...
    into a json string.
    This is useful for example if we want to use this request with the JSON formatter.
    ...
    def _jsonify_request(self, record):
        if hasattr(record.request, '__dict__'):
        ...
        if isinstance(record.request, HttpRequest) and self._add_key('request.headers', 'headers'):
